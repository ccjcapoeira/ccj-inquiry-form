...
(async()=>{
 ...
 const source=fs.readFileSync(path.join(root,'index.html'),'utf8').replace("const SCRIPT_URL = '';","const SCRIPT_URL = 'https://example.invalid/mock';");
 ...
 const clean=await browser.newContext({viewport:{width:390,height:844},locale:'ja-JP'});const visual=await clean.newPage();await visual.goto(base);
 ...
 const unavailable=await browser.newContext();await unavailable.addInitScript(()=>Object.defineProperty(window,'localStorage',{get(){throw new Error('使用不可');}}));const blocked=await unavailable.newPage();await blocked.goto(base+'?utm_source=test');const empty=await blocked.evaluate(()=>collectPayload());for(const k of ['utm_source','utm_medium','utm_campaign','utm_content','utm_term','firstLandingPage','firstReferrer'])assert.equal(empty[k],'');assert.equal(empty.submitPage,base+'?utm_source=test');
 const nojs=await browser.newContext({javaScriptEnabled:false});const plain=await nojs.newPage();await plain.goto(base);assert(await plain.locator('#name').isVisible());assert(await plain.locator('noscript').isVisible());assert(await plain.locator('#review').isDisabled());
 ...
 const redirectContext=await browser.newContext();await redirectContext.route('https://**/*',route=>route.fulfill({contentType:'text/html',body:'<!doctype html><title>転送先確認</title>'}));const redirectPage=await redirectContext.newPage();
 ...
 const nojsRedirect=await browser.newContext({javaScriptEnabled:false});await nojsRedirect.route('https://**/*',r=>r.fulfill({contentType:'text/html',body:'転送確認'}));const meta=await nojsRedirect.newPage();await meta.goto(base+'go/sample-event.html');await meta.waitForURL('https://ccjcapoeira.github.io/ccj-inquiry-form/?utm_source=event&utm_medium=qr&utm_campaign=sample-event');
